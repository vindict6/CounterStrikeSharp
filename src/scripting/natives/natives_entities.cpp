...
void EntityKeyValuesRelease(ScriptContext& script_context)
{
    CEntityKeyValues* keyValues = script_context.GetArgument<CEntityKeyValues*>(0);
    if (!keyValues)
    {
        script_context.ThrowNativeError("Invalid KeyValues pointer");
        return;
    }
...
REGISTER_NATIVES(entities, {
    ...
    ScriptEngine::RegisterNativeHandler("ENTITY_KEY_VALUES_RELEASE", EntityKeyValuesRelease);
